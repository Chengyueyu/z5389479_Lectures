...
def get_tics(pth):
    ...
    with open(pth, mode='r') as fobj:
        dict = {}
        for L in fobj:
            exchange, tic = (L.strip().replace("\"", '').lower().split('='))
            dict[tic] = exchange
        return dict
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    if tickers_lst is None:
        pass
    elif len(tickers_lst) == 0:
        raise Exception('tickers_lst is an empty list.')
    elif len(tickers_lst) > 0:
        for c in tickers_lst:
            if c in list(tic_exchange_dic.keys()):
                continue
            else:
    ...
    if col_lst is None:
        ...
        for d in col_lst:
            if d in COLUMNS:
                ...
                raise Exception('col_lst contains a ticker that does not correspond to a key tic_exchange_dic')
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    if col_lst is None:
        col_lst = COLUMNS
    dict = {}
    for m in tickers_lst:
        tic_dict = {}
        tic_dict['exchange'] = tic_exchange_dic[m]
        data = []
        lines = read_dat(m)
        for n in lines:
            A = {}
            for col in col_lst:
                A[col] = line_to_dict(n)[col]
            data.append(A)
        tic_dict['data'] = data
        dict[m] = tic_dict
    return dict
